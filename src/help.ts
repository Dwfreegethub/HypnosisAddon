...
function lastingLines(): HelpLine[] {
	...
	return [
		...
		gap(),
		head("If you are silenced and something is stuck"),
		body("/hypno release [n]   undoes a fired trigger, keeps it planted"),
		body("/hypno forgettrigger [n]   undoes it and deletes it"),
		body("/hypno safeword   clears absolutely everything"),
		dim("Chat commands survive speech blocking; ordinary speech does not,"),
		dim("so a spoken release is not available to you while muted."),
		gap(),
