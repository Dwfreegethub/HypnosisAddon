...
5. ~~**Session state recovery after disconnect**~~ — **done v0.44.0** (`recovery.ts`). DW's rules: under 5 minutes with the hypnotist present or returning, resume where it left off; over 5 minutes, the trance breaks; a fired trigger with time left serves out its **remainder** regardless, because a trigger was never session state to begin with; and *Release everything if you disconnect* opts out of all of it.
   ...
   **The illusion is deliberately not restored.** Re-freezing on reconnect would snapshot the appearance as it is at that moment — the truth — which is the same trap `carry.ts` documents for waking. Rebuilding the original snapshot means serialising and re-resolving asset definitions, which is real work for the least common state, and the failure direction matters: after a disconnect, defaulting to the truth about your own body is the safer way to be wrong. The subject is told, and a hypnotist who wants it back says one line.
