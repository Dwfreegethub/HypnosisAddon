...
One of mods you are using is using an old version of SDK. It will work for now but please inform author to update`), window.bcModSdk);
  ...
  function depthRefusal(key, full = currentDepth(), earned = currentDepthEarned()) {
    ...
    const have = gate?.earnedOnly ? earned : full;
    return `needs ${tierLabel(need)} (${requiredDepth(key)}), at ${have.toFixed(0)}${gate?.earnedOnly ? " earned \u2014 arousal does not count toward this one" : ""}`;
  }
  ...
  function inductionChance(hypnotistId, choice, earnedOnly = false, skill = NO_SKILL) {
  ...
  function describeChances(memberId) {
    ...
    const exp = experienceValue();
    const perSession = (c) => 100 * (1 - Math.pow(1 - c / 100, MAX_ATTEMPTS));
    return [
  ...
  function showPrompt(hypnotistName) {
    ...
    notify(
      `${hypnotistName} is attempting to hypnotize you. Choose on the box in the room, or with /hypno agree, /hypno ignore, /hypno fight \u2014 they will not be told which you chose. (${Math.round(PROMPT_TIMEOUT_MS / 1e3)}s)`
    );
  }
  ...
  var TRIGGER_TRUST_THRESHOLD = 65;
  var MIN_PHRASE_LENGTH = 3;
  var MAX_ACTIONS = 8;
  ...
  function ageTriggers(days, index) {
    ...
    if (!TESTING_MODE) return refuse2("not available \u2014 this build is not in testing mode");
  ...
  function beginRecording(hypnotistId, hypnotistName, phrase) {
    const refuse2 = (why) => {
    ...
    if (phrase.length < MIN_PHRASE_LENGTH) {
      return refuse2(`[trigger] Refused \u2014 "${phrase}" is too short to use as a trigger.`);
    }
  ...
  var ILLUSION_TRUST_THRESHOLD = 65;
  var SUGGESTIONS = [
  ...
  function fireTrigger(trigger) {
    ...
    log(
      `trigger "${trigger.phrase}" fired ${fired}/${trigger.actions.length} actions at strength ${strength}${tooWeak ? ` (${tooWeak} too weak)` : ""}`
    );
    noteTriggerFired(trigger);
    if (fired) {
  ...
  function installCommands() {
    const summary = ["Session", "Diagnostics", "Data", "Testing"].map((g) => {
    ...
    CommandCombine({
      ...
      Action: () => {
        reply(
          "Most of this add-on is used by SPEAKING to someone during a session, not by command. The full guide \u2014 what to say, how trust works, every command \u2014 is under Preferences > Extensions > Hypnosis Add-on > Help, and on the Help button in the remote panel."
        );
        reply(summary);
  ...
  var COLUMN_LEFTS = [CONTENT_LEFT, CONTENT_LEFT + 640];
  ...
  function drawHelpLines(lines, page2) {
    ...
    const start = Math.min(page2, pages - 1) * perPage;
  ...
  function startedLines() {
    return [
      ...
      gap(),
      head("Getting out"),
      body("Wake Up button \xB7 a spoken wake word \xB7 /hypno wake if the"),
      body("trance is shallow \xB7 a 30 minute timeout \xB7 /hypno safeword,"),
  ...
  function vocabularyLines() {
    ...
    for (const s of suggestionHelp()) {
      ...
      lines.push({
        text: `${s.examples.map((e) => `"${e}"`).join("  \xB7  ")}${gate}`,
        style: s.release ? "dim" : "body"
      });
    }
    lines.push(gap());
    lines.push(head("Not in the table"));
    lines.push(body(`"wake up" \xB7 "you are awake" \xB7 "come back to me"`));
    lines.push(dim("   always allowed \u2014 ending a trance answers to no permission"));
    lines.push(body(`"you cannot touch your breasts" \xB7 "...yourself"`));
  ...
  function trustLines() {
    return [
      ...
      body("Friend 15 \xB7 Lover 30 \xB7 Owner 65, under whatever you have"),
      ...
      body("Stats tab: Never \xB7 Very slowly \xB7 Slowly \xB7 Typical \xB7"),
      body("Fast \xB7 Very fast. Off unless you pick otherwise."),
  ...
  function installMenu() {
    PreferenceRegisterExtensionSetting({
      ...
      run: () => {
        ...
        drawTabsAndPanel(TABS2.map((t) => t.name), activeTab2);
        const tab = TABS2[activeTab2];
        const locked = settingsLocked();
        drawLeftTextFit(
      ...
      click: () => {
        ...
        const hitTab = tabHitIndex(TABS2.length);
        if (hitTab !== null) {
          activeTab2 = hitTab;
          depthPage = 0;
          removeScopeControl();
        ...
        if (TABS2[activeTab2].clickExtra?.()) return;
        if (TABS2[activeTab2].render) {
          if (MouseIn(PAGE_PREV_LEFT2, PAGE_BUTTON_TOP2, PAGE_BUTTON_WIDTH2, PAGE_BUTTON_HEIGHT2)) {
        ...
        const features = getFeatures();
        const clickRows = TABS2[activeTab2].rows ?? [];
  ...
  function drawPrompt(hypnotistName, remainingMs) {
    ...
    DrawTextFit("They are never told which you choose.", centre, PANEL_TOP3 + 110, inner, "Gray");
    DrawTextFit(
  ...
  function showIndicator() {
    ...
    el.textContent = `Hypnosis Add-on v${"0.64.0"} loaded`;
