// Erotic Chat Hypnosis Suite (ECHS) v0.92.1. Loaded at runtime by the installed loader;
...
One of mods you are using is using an old version of SDK. It will work for now but please inform author to update`), window.bcModSdk);
  ...
  var ownPoseChange = 0;
  var lastHeldNotice = 0;
  var heldPose = null;
  var HOLD_PRIORITY = 1e3;
  function noteHeldPose() {
  ...
  function samePoses(a, b) {
    const norm = (p) => (Array.isArray(p) ? p : typeof p === "string" && p ? [p] : []).slice().sort().join(",");
    return norm(a) === norm(b);
  }
  function poseWouldChange(pose, force) {
    const now = currentPoses();
  ...
  function installEffectHooks(modApi2) {
    ...
    modApi2.hookFunction("ChatRoomSyncPose", HOLD_PRIORITY, (args, next) => {
      const [data] = args;
    ...
    modApi2.hookFunction("PoseCanChangeUnaidedStatus", HOLD_PRIORITY, (args, next) => {
  ...
  function installMenu() {
    PreferenceRegisterExtensionSetting({
      ...
      run: () => {
        ...
        DrawText(`Erotic Chat Hypnosis Suite (ECHS) v${"0.92.1"} \u2014 settings`, MainCanvasWidth / 2, TITLE_Y, "Black");
        DrawButton(BACK_LEFT, BACK_TOP, BACK_SIZE, BACK_SIZE, "", "White", "Icons/Exit.png", "Exit");
  ...
  function showStartupBanner() {
    ...
    bannerShown = true;
    tellPlayer(`Erotic Chat Hypnosis Suite (ECHS) \xB7 v${"0.92.1"} \xB7 /hypno help`);
  }
  ...
  function showLoadedToast() {
    ...
    el.textContent = `ECHS v${"0.92.1"} loaded`;
